...
const displayComments = () => {
  ...
  // 리뷰들을 가져오기 위해 movie 객체 내의 comments 배열을 참조합니다.
  ...
  const commentsHTML = comments.map((comment, index) => {
    return `
      <div class="user-review">
        <p class="writer">${comment.name}</p>
        <p class="review-comment" id="review-comment">${comment.review}</p>
        <div class="edit-box">
          <div class="btns">
...
function pwCheck(index, password) {
  ...
  const storedPassword = comment.pw; // 리뷰 객체에서 비밀번호 가져오기

  return password === storedPassword; // 비밀번호 일치 여부 반환
}

// 삭제 버튼 클릭시 deleteCheck 함수 실행, index : 버튼의 index
